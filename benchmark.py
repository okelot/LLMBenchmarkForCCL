...
def load_cases(csv_path: str = "random_cases.csv", max_cases: Optional[int] = None) -> List[Dict]:
    """Load human-annotated case briefs from the scraped CSV.

    Pass `max_cases` to load only the first N cases (handy for quick tests).
    """
    cases: List[Dict] = []
...
def build_prompt(case_name: str, citation: str = "") -> str:
    ...
    return (
        ...
        f"{{{keys}}}. Output only the JSON, with no markdown, commentary, or "
        f'surrounding text. If you do not know this specific case, respond with '
        f'exactly: {UNKNOWN_SENTINEL}'
    )


def get_case_brief(case_name: str, llm: OpenRouterWrapper, citation: str = "") -> Dict[str, str]:
    """Ask the model for a brief and return one entry per section.

    On a recognised "I don't know" answer every section is set to the sentinel;
    on a parse failure every section is set to "ERROR".
    """
    response = llm.invoke(build_prompt(case_name, citation), context=SYSTEM_PROMPT)
    try:
        parsed = json.loads(make_valid_json(response))
        return {f"ai_{s}": str(parsed.get(s, "")) for s in SECTIONS}
    except (ValueError, json.JSONDecodeError):
        fill = UNKNOWN_SENTINEL if UNKNOWN_SENTINEL.lower() in response.lower() else "ERROR"
        return {f"ai_{s}": fill for s in SECTIONS}
...
def run_benchmark(
    ...
    print(f"Loaded {len(models)} models and {len(cases)} cases")
    ...
    with open(output_path, "w", newline="", encoding="utf-8") as csvfile:
        ...
        for config in models:
            ...
            for case in cases:
                ...
                try:
                    ai_output = get_case_brief(case_name, llm, case["citation"])
                except Exception as e:
                    print(f"    Error on {case_name} with {model_id}: {e}")
                    continue

                row = {"Model_ID": model_id, "Case_Name": case_name}
